 ...
 * Main plugin class.
 *
 * @package LSX_TO_Settings
 * @author  LightSpeed
 */
class LSX_TO_Settings extends Tour_Operator {
	...
	public function welcome_page() {
	    include( LSX_TO_PATH . 'includes/settings/welcome.php' );
	}
		...
		if(false !== $post_types && !empty($post_types)){
			foreach($post_types as $index => $title){
				...
				$tabs[$index] = array(
					...
					'page_description'  => '',
					'menu_title'        => $title,
					'template'          => apply_filters('lsx_to_settings_path',LSX_TO_PATH,$index).'includes/settings/'.$index.'.php',
					'default'	 		=> false,
					'disabled'			=> $disabled
				);
			}
	...
	public function settings_page_array(){
		...
		$tabs = array( // tabs array are for setting the tab / section templates
				...
				'general'		=> array(
						...
						'template'          => LSX_TO_PATH.'includes/settings/general.php',
						'default'	 		=> true
				)
		);
		$tabs['display'] = array(
		...
		if(false === $posts_page){
			$tabs['post'] = array(
				...
				'template'          => LSX_TO_PATH.'includes/settings/post.php',
				'default'	 		=> false
			);
		}
