...
#### Test tooling
- **WordPress integration tests now run in CI** - the two integration suites were excluded from every working configuration and so ran nowhere. A `wp-integration` job now runs them against a MariaDB service container, with WordPress core and its PHPUnit test suite supplied by Composer (`roots/wordpress-no-content` and `wp-phpunit/wp-phpunit`), so the versions are pinned in `composer.lock` and the runner needs no download step and no Subversion.
- **Removed three test scripts that could report success without testing anything** - `bin/install-wp-tests.sh` was not the canonical WP-CLI script: it fetched three files from the WordPress test suite and, on failure, called `create_minimal_test_suite()`, which writes stub `add_filter()` and `tests_add_filter()` implementations that return `true`. A suite bootstrapped that way reports passing tests while asserting nothing against real WordPress. `bin/run-wp-tests.sh` then mapped PHPUnit exit code 1 - genuine test failures - onto exit 0. Both are gone, along with the unused `bin/setup-wp-test-env.sh`.
- **Integration suite pinned to PHPUnit 9** - the WordPress core test suite supports PHPUnit 8 and 9 only: `WP_UnitTestCase_Base::expectDeprecated()` calls `PHPUnit\Util\Test::parseTestMethodAnnotations()`, removed in PHPUnit 10, so every test errors on 10 ([Trac #59486](https://core.trac.wordpress.org/ticket/59486)). The Composer constraint now allows either major; the standalone suite keeps running on PHPUnit 10 and the integration job resolves 9.6 for its own checkout.
- **Test bootstrap pinned a stale plugin version** - it defined `LSX_TO_PATH`, `LSX_TO_URL` and `LSX_TO_VER` before requiring the plugin. Since the plugin guards its own defines, the bootstrap's values won, fixing `LSX_TO_VER` at a hardcoded `2.1.0` for the whole test run regardless of the real version.
- **Test helper fatal and stale assertions** - `Tour_Operator_Test_Case::clean_up_global_scope()` was declared `protected` over a `public` parent, which is a fatal error, and replaced core's teardown instead of extending it. `RegistrationTest` called a non-existent `get_all_block_bindings_sources()` (the core function is `get_all_registered_block_bindings_sources()`) and asserted a hardcoded pattern list that had drifted from the `patterns/` directory - it expected `lsx-tour-operator/gallery` where the file is `gallery-section.php`, and omitted `section-header` entirely. It now derives the expected slugs from that directory so it cannot drift again.
