...
(function ($, window, document, undefined) {
  ...
  lsx_to.set_read_more = function () {
    ...
    $('.tax-travel-style .wp-block-read-more, .tax-accommodation-brand .wp-block-read-more, .tax-accommodation-type .wp-block-read-more').each(function () {
      if (0 < $(this).parents('.wp-block-post-content.term-description').length) {
        console.log(this);
        lsx_to.readMoreText = $(this).contents().filter(function () {
          return this.nodeType === Node.TEXT_NODE;
        }).text();
        ...
        lsx_to.readMoreSet(newMore, newMore.parents('.wp-block-post-content.term-description'), pIndex);
      }
    });
    ...
    $('.tax-travel-style .wp-block-read-more, .tax-accommodation-brand .wp-block-read-more, .tax-accommodation-type .wp-block-read-more').on('click', function (event) {
      event.preventDefault();
      if (0 < $(this).parents('.wp-block-post-content.term-description').length) {
        $(this).hide();
        let pIndex = $(this).parents('.wp-block-post-content.term-description').data('readmore-index');
        if ($(this).hasClass('less-link')) {
          lsx_to.readMoreSet($(this), $(this).parents('.wp-block-post-content.term-description'), pIndex);
        } else {
          lsx_to.readMoreOpen($(this), $(this).parents('.wp-block-post-content.term-description'));
        }
        $(this).show();
        $(this).parent('p').show();
      }
  ...
  lsx_to.build_slider_lightbox = function () {
    ...
    if (0 < $('.lsx-units-wrapper .unit-image a').length) {
      const roomImages = $('.lsx-units-wrapper .unit-image a img').map(function () {
