...
(function (wp) {
  ...
  const StickyToggle = () => {
    ...
    const isSticky = useSelect(function (select) {
      return select('core/editor').getEditedPostAttribute('meta')['featured'];
    }, []);
  ...
  const DisableSingleToggle = () => {
    ...
    const handleChange = newChecked => {
      editPost({
        meta: {
          'disable_single': newChecked
        }
      });
    };
    const isSticky = useSelect(function (select) {
      return select('core/editor').getEditedPostAttribute('meta')['disable_single'];
    }, []);
    return createElement(ToggleControl, {
  ...
  const LsxToStatusPlugin = function () {
    ...
    }, createElement(StickyToggle)), createElement('div', {
      className: 'toggle-row'
