...
class Admin extends Tour_Operator {
	...
	public function __construct() {
		...
		add_action( 'cmb_save_custom', array(
			$this,
			'post_relations',
		), 3, 20 );

		add_filter( 'plugin_action_links_' . plugin_basename( LSX_TO_CORE ), array(
			$this,
			'add_action_links',
		) );

		add_action( 'default_hidden_meta_boxes', array(
			$this,
	...
	public function init() {
		if ( is_admin() ) {
			...
			add_filter( 'lsx_to_taxonomy_widget_taxonomies', array(
				$this,
				'widget_taxonomies',
			), 10, 1 );
			...
			add_filter( 'type_url_form_media', array(
				$this,
				'change_attachment_field_button',
			), 20, 1 );
		}
	}
	...
	public function global_taxonomies() {
		...
		register_taxonomy( 'travel-style', array(
			'accommodation',
			'tour',
			'destination',
			'review',
	...
	public function add_thumbnail_form_field( $term = false ) {
        ...
        <tr class="form-field form-required term-thumbnail-wrap">
            <th scope="row"><label
                        for="thumbnail"><?php esc_html_e( 'Featured Image', 'tour-operator' ); ?></label>
            </th>
            <td>
                <input class="input_image_id" type="hidden" name="thumbnail"
                       value="<?php echo wp_kses_post( $value ); ?>">
                <div class="thumbnail-preview">
					<?php echo wp_kses_post( $image_preview ); ?>
                </div>
                <a style="<?php if ( '' !== $value && false !== $value ) { ?>display:none;<?php } ?>"
                   class="button-secondary lsx-thumbnail-image-add"><?php esc_html_e( 'Choose Image', 'tour-operator' ); ?></a>
                <a style="<?php if ( '' === $value || false === $value ) { ?>display:none;<?php } ?>"
                   class="button-secondary lsx-thumbnail-image-remove"><?php esc_html_e( 'Remove Image', 'tour-operator' ); ?></a>
	...
	public function add_tagline_form_field( $term = false ) {
        ...
        <tr class="form-field form-required term-tagline-wrap">
            <th scope="row"><label
                        for="tagline"><?php esc_html_e( 'Tagline', 'tour-operator' ); ?></label>
            </th>
            <td>
                <input name="tagline" id="tagline" type="text"
                       value="<?php echo wp_kses_post( $value ); ?>" size="40"
                       aria-required="true">
            </td>
