...
class Tour {
	...
	private function __construct() {
		...
		add_filter( 'lsx_to_search_fields', array(
			$this,
			'single_fields_indexing',
		) );

		include( 'class-itinerary.php' );

		add_action( 'lsx_to_framework_tour_tab_general_settings_bottom', array(
			$this,
			'general_settings',
		), 10, 1 );

		add_filter( 'lsx_to_itinerary_class', array(
			$this,
			'itinerary_class',
		) );
		add_filter( 'lsx_to_itinerary_needs_read_more', array(
			$this,
			'itinerary_needs_read_more',
		) );
	...
	public function content_meta() {
		if ( 'tour' === get_post_type() ) {
            ...
            <div class="tour-details">
                <div class="meta info"><?php lsx_to_price( '<span class="price">' . esc_html__( 'From price', 'tour-operator' ) . ': ', '</span>' );
					lsx_to_duration( '<span class="duration">' . esc_html__( 'Duration', 'tour-operator' ) . ': ', '</span>' ); ?></div>
