...
function lsx_tour_best_time_to_visit($before="",$after="",$echo=true){
	...
	if(false !== $best_time_to_visit && '' !== $best_time_to_visit && is_array($best_time_to_visit) && !empty($best_time_to_visit)){
		...
		foreach($this_year as $month => $label){
			$checked = '';
			if(in_array($month,$best_time_to_visit)){ $checked = '<span class="genericon genericon-checkmark"></span>'; }
...
function lsx_vehicle_code($before="",$after="",$echo=true){
	lsx_custom_field_query('code',$before,$after,$echo);
}
/**
...
function lsx_vehicle_engine_type($before="",$after="",$echo=true){
	lsx_custom_field_query('engine_type',$before,$after,$echo);
}
/**
...
function lsx_vehicle_gearbox($before="",$after="",$echo=true){
	lsx_custom_field_query('gearbox',$before,$after,$echo);
}
/**
...
function lsx_vehicle_engine_size($before="",$after="",$echo=true){
	lsx_custom_field_query('engine_size',$before,$after,$echo);
}
/**
...
function lsx_vehicle_gears($before="",$after="",$echo=true){
	lsx_custom_field_query('gears',$before,$after,$echo);
}
/**
...
function lsx_vehicle_seating($before="",$after="",$echo=true){
	lsx_custom_field_query('seating',$before,$after,$echo);
}
...
function lsx_has_vehicle_features(){
	return lsx_has_custom_field_query( 'features', get_the_ID() );
}
/**
...
function lsx_vehicle_features(){
	if(is_singular('vehicle')){
		$features = lsx_custom_field_query('features',"","",false);
		if(null !== $features) { ?>
			<section id="features">
				<div class="row">						
					<?php if(null !== $features) { ?>
						<div class="panel col-sm-12">
							<h2 class="section-title"><?php _e('Features','lsx-tour-operators'); ?></h2>
							<div class="entry-content">
								<?php echo apply_filters('the_content',wpautop($features)); ?>
							</div>
						</div>
					<?php } ?>
				</div>
 ...
 * @category 	review
 */
function lsx_review_rating($before="",$after="",$echo=true){
	lsx_custom_field_query('rating',$before,$after,$echo);
}
 ...
 * @category 	review
 */
function lsx_review_dates($before="",$after="",$echo=true){
 ...
 * @category 	review
 */
function lsx_review_accommodation(){
	global $lsx_archive;
 ...
 * @category 	review
 */
function lsx_review_destination(){
	global $lsx_archive;
	if(post_type_exists('destination') && is_singular('review')) {
		$args = array(
				...
				'to'			=>	'review',
				'column'		=>	'12',
				'before'		=>	'<section id="destinations"><h2 class="section-title">'.__(lsx_get_post_type_section_title('destination', '', 'Featured Destinations'),'lsx-tour-operators').'</h2>',
				'after'			=>	'</section>'
		);
		lsx_connected_panel_query($args);
	}
