...
function Edit({ attributes, setAttributes, clientId }) {
    ...
    const blockProps = useBlockProps({
        style: {
            '--active-bg-color': getColorValue(
                activeBackgroundColor,
                customActiveBackgroundColor
            ),
            '--active-text-color': getColorValue(
                activeTextColor,
                customActiveTextColor
            ),
            '--hover-bg-color': getColorValue(
                hoverBackgroundColor,
                customHoverBackgroundColor
            ),
            '--hover-text-color': getColorValue(
                hoverTextColor,
                customHoverTextColor
            ),
        },
    ...
    useEffect(() => {
        const timeoutId = setTimeout(() => {
            const find_sticky_menu_blocks = (blocks) => {
                ...
                blocks.forEach((block) => {
                    ...
                    if (
                        ...
                        const sectionTitle =
                            block.attributes.metadata?.name || sectionId;
                    ...
                    if (block.innerBlocks && block.innerBlocks.length > 0) {
                        sticky_blocks = sticky_blocks.concat(
                            find_sticky_menu_blocks(block.innerBlocks)
                        );
                    }
                });
            ...
            const current_ids = menuItems
                .map((item) => `${item.id}:${item.title}`)
                .sort()
                .join('|');
            const new_ids = new_menu_items
                .map((item) => `${item.id}:${item.title}`)
                .sort()
                .join('|');
    ...
    return (
        <>
            ...
            <div
                {...blockProps}
                style={wrapperStyle}
                data-sticky-offset-selector={stickyOffsetSelector || undefined}
            >
                <nav
                    className="lsx-to-sticky-menu-nav is-layout-constrained has-global-padding"
                    aria-label={__('Page section navigation', 'tour-operator')}
                >
                    {menuItems.length > 0 ? (
                        <>
                            <ul
                                className="lsx-to-sticky-menu-list alignwide"
                                aria-label={__(
                                    'Page sections',
                                    'tour-operator'
                                )}
                        ...
                        <div className="lsx-to-sticky-menu-placeholder">
                            <p>
                                {__(
                                    'Add section groups with "Add to Sticky Menu" enabled to populate this menu.',
                                    'tour-operator'
                                )}
                            </p>
                            <details
                            ...
                            >
                                ...
                                >
                                    <li>
                                        {__(
                                            'Add a Group block and set HTML element to "section"',
                                            'tour-operator'
                                        )}
                                    </li>
                                    <li>
                                        {__(
                                            'Set an HTML Anchor in Block Settings → Advanced',
                                            'tour-operator'
                                        )}
                                    </li>
                                    <li>
                                        {__(
                                            'Name the block in the List View or toolbar',
                                            'tour-operator'
                                        )}
                                    </li>
                                    <li>
                                        {__(
                                            'Enable "Add to Sticky Menu" in the block sidebar',
                                            'tour-operator'
                                        )}
                                    </li>
                                    <li>
                                        {__(
                                            'The menu will automatically populate with the block name and anchor',
                                            'tour-operator'
                                        )}
                                    </li>
                                </ol>
        ...
        >
            ...
            >
                {menuItems.length > 0 && (
                    <ul
                        className="lsx-to-sticky-menu-list alignwide"
                        aria-label={__('Page sections', 'tour-operator')}
                    >
                        {menuItems.map((item) => (
                            <li
                                key={item.id}
