...
<article id="term-<?php echo esc_attr( $term->term_id ); ?>" class="term-<?php echo esc_attr( $term->term_id ); ?> <?php echo esc_attr( $taxonomy ); ?> type-<?php echo esc_attr( $taxonomy ); ?> status-publish hentry">
 	<?php if('1' !== $disable_placeholder && true !== $disable_placeholder) { ?>
		...
		<?php else: ?>	
			<div class="thumbnail">
				<a href="<?php echo esc_url( home_url($taxonomy.'/'.$term->slug.'/') ); ?>">
					<img alt="Placeholder" class="attachment-responsive wp-post-image lsx-responsive" src="<?php echo esc_attr( LSX_Placeholders::placeholder_url() . ( parse_url( LSX_Placeholders::placeholder_url(), PHP_URL_QUERY ) ? '&' : '?' ) ); ?>w=350&amp;h=230">
				</a>	
			</div>				
		<?php endif; ?>
	<?php } ?>
	...
	<div class="widget-content">
		<div class="view-more" style="text-align:center;">
			<a href="<?php echo esc_url( home_url($taxonomy.'/'.$term->slug.'/') ); ?>" class="btn btn-primary text-center"><?php esc_html_e('View','lsx-tour-operators'); ?> <?php echo esc_html( apply_filters('the_title', $term->name) ); ?> <?php esc_html_e('tours','lsx-tour-operators'); ?></a>
		</div>	
	</div>
